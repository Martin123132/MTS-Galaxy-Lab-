(function () {
  ...
  var V17STATE_EXACT_TOKEN = "__MTS_V17_73_LOWLOAD_EDGE_LAW__";
  ...
  var FRAMEWORK_PRESET_LABELS = {
    mts: "MTS baseline",
    v17state: "MTS v17.73 low-load edge candidate",
    baryon: "Baryon only",
  ...
  function v17ExactCacheEntry(curve) {
    var cache = window.MTS_V17_73_SUPPORT_CACHE;
  ...
  function updateFrameworkPanel() {
    ...
    if (batch) {
      note.textContent = (state.framework.compiled && state.framework.compiled.kind === "v17state-exact-cache" ? "v17.73 exact cache active. " : "") +
        "Batch complete: " + batch.wins + " / " + batch.count + " LTGs beat baseline. Median delta " + fmt(batch.medianDelta, 2) + " km/s." +
        (batch.exactCacheFallbacks ? " v17.73 exact cache fallback used on " + batch.exactCacheFallbacks + " point(s)." : "");
    } else if (score) {
      if (score.exactCacheHits) {
        note.textContent = "v17.73 exact cached candidate active for this built-in curve. " +
  ...
  function runFrameworkBatch() {
    ...
    $("frameworkNote").textContent = (state.framework.compiled && state.framework.compiled.kind === "v17state-exact-cache" ? "v17.73 exact cache active. " : "") +
