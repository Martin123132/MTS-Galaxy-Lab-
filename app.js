(function () {
  ...
  var V17STATE_EXACT_TOKEN = "__MTS_V17_76_NEGATIVE_CURVATURE_STATE_GATE__";
  ...
  var FRAMEWORK_PRESET_LABELS = {
    mts: "MTS baseline",
    v17state: "MTS v17.76 negative-curvature state gate",
    baryon: "Baryon only",
  ...
  function compileFrameworkExpression(expression) {
    ...
    if (source === V17STATE_EXACT_TOKEN) {
      return {
        source: source,
  ...
  function v17ExactCacheEntry(curve) {
    var cache = window.MTS_V17_76_SUPPORT_CACHE;
  ...
  function updateFrameworkPanel() {
    ...
    if (batch) {
      note.textContent = (state.framework.compiled && state.framework.compiled.kind === "v17state-exact-cache" ? "v17.76 exact cache active. " : "") +
        "Batch complete: " + batch.wins + " / " + batch.count + " LTGs beat baseline. Median delta " + fmt(batch.medianDelta, 2) + " km/s." +
        (batch.exactCacheFallbacks ? " v17.76 exact cache fallback used on " + batch.exactCacheFallbacks + " point(s)." : "");
    } else if (score) {
      if (score.exactCacheHits) {
        note.textContent = "v17.76 exact cached candidate active for this built-in curve. " +
  ...
  function runFrameworkBatch() {
    ...
    $("frameworkNote").textContent = (state.framework.compiled && state.framework.compiled.kind === "v17state-exact-cache" ? "v17.76 exact cache active. " : "") +
